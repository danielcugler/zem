...
$(document)
		.ready(
				function() {
					...
					searchToken(URL + '/search/1', JSON.stringify({
						'callSource' : [],
						'callProgress' : [],
						'priority' : [],
						'classification' : [],
					...
					/*
					 * $.ajax( { type : 'POST', url :
					 * '../rest/callminitor/search/1' + $('#username').text(),
					 * dataType : "json", data : JSON .stringify({ 'callSource' :
					 * [], 'callProgress' : [], 'priority' : [],
					 * 'classification' : [], 'entity' : [], 'orderParam' : 4,
					 * 'order' : 1 }), beforeSend : function(xhr) {
					 * xhr.setRequestHeader(header, token); }, contentType :
					 * "application/json" }) .done( function(data) {
					 * template(data2, templateHandlebars, selector); count =
					 * data2.count; valoresGraficos(JSON.parse(data2.graph));
					 * }).fail();
					 * 
					 */
