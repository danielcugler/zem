var ctrlEC = (function($, module) {
	...
	function tableTpl(roleEdit, roleEnabled) {
		var string = "{{#each .}}" + "<tr>"
				+ "<td class=\"hideTD\">{{entityCategoryId}}</td>"
				+ "<td>{{name}}</td>	"
				+ "{{#ifCond send_message '===' 'ENABLED'}}" + "<td align='center'>Sim</td>"
				+ "{{else}}" + "<td align='center'>Não</td>" + "{{/ifCond}}"
				+ "<tdclass=\"hideTD\">{{enabled}}</td>"
				+ "<td align='right'> <div class='btn-group' role='group' aria-label='...'>	";
		if (roleEdit)
			string += "<a href=\"../addec.jsp?id={{entityCategoryId}}\" type = 'button' target = '_self' data-toggle='tooltip' title='Editar' class='btn btn-xs btn-primary edit-button'><i class='glyphicon glyphicon-pencil'></i></a>";
		if (roleEnabled)
			string += "{{#ifCond enabled '===' 0}}"
					+ "<a type = 'button' data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{entityCategoryId}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" data-toggle='tooltip' title='Inativar' class='btn btn-xs btn-danger ed-button enabled-button'><i class='glyphicon glyphicon-remove'></i></a>"
					+ "<a type = 'button' data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{entityCategoryId}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" style=\"display: none;\" data-toggle='tooltip' title='Ativar' class='btn btn-xs btn-success ed-button disabled-button'><i class='glyphicon glyphicon-ok'></i></a>"
					+ "{{else}}"
					+ "<a type = 'button' data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{entityCategoryId}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" style=\"display: none;\" data-toggle='tooltip' title='Inativar' class='btn btn-xs btn-danger ed-button enabled-button'><i class='glyphicon glyphicon-remove'></i></a>"
					+ "<a type = 'button'  data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{entityCategoryId}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" data-toggle='tooltip' title='Ativar' class='btn btn-xs btn-success ed-button disabled-button'><i class='glyphicon glyphicon-ok'></i></a>"
					+ "{{/ifCond}}";
	...
	function searchMM(page,access) {
		...
		$
				.ajax({
					type : 'GET',
					url : "../rest/entitycategory2/search",
				...
				.done(
						...
						function(jqXHR, textStatus) {
							...
							*/
								...
								var title = "Categoria de Entidade";
	...
	function changeEnabled(event) {
		...
		$.ajax({
			...
			modalED("Categoria de Entidade", resp.message);
