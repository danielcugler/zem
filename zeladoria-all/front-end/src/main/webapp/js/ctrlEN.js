var ctrlEN = (function($, module) {
	...
	function tableTpl(roleEdit, roleEnabled) {
		var string = "{{#each .}}" + "<tr>"
				...
				+ "<td>{{name}}</td>	"
				+ "<td align='center'>{{attendanceTime.lowPriorityTime}}</td>	"
				+ "<td align='center'>{{attendanceTime.mediumPriorityTime}}</td>	"
				+ "<td align='center'>{{attendanceTime.highPriorityTime}}</td>	"
				+ "<tdclass=\"hideTD\">{{enabled}}</td>"
				+ "<td align='right'> <div class='btn-group' role='group' aria-label='...'>	";
		if (roleEdit)
			string += "<a href=\"../adden.jsp?id={{entityId}}\" type = 'button' target = '_self' data-toggle='tooltip' title='Editar' class='btn btn-xs btn-primary edit-button'><i class='glyphicon glyphicon-pencil'></i></a>";
		if (roleEnabled)
...
*/
	...
	function changeEnabled(event) {
		...
		// var body = (enabled == "Enabled") ? "Entidade " + name
		// + " inativado com sucesso!" : "Entidade " + name
		// + " ativado com sucesso!";
		$.ajax({
			type : 'PUT',
			url: '../rest/entity2/ed/' + id,
			dataType : "json",
