var ctrlMONT = (function($, module) {
	...
	var roleEdit = $.inArray('ROLE_CALLMONITOR_UPDATE', list) >= 0;
	// cached DOM
	// general module
	...
	function tableTpl() {
		var string = "{{#each .}}"
				...
				// data
				if (roleEdit)
				string += "<td><select data-id='{{unsolvedCallId}}' class=\"form-control selectPriority selectpicker\">";
				else
				string += "<td><select data-id='{{unsolvedCallId}}' class=\"form-control selectPriority selectpicker\" disabled>";
				string += "{{#ifCond priority '===' 'Baixa'}}"
	...
	function sortColumn(event) {
		if (!$(event.target).is('.btn-group') || !$(event.target).is('.dropdown-menu'))
	...
	function changeOrder(event){
		if (!$(event.target).is('.btn-group, .icheckbox_square-aero, ul, li')) {
	...
	// Método responsável por exibir os checkboxes com o CSS do plugin iCheck
													...
													for (; targetColumn < 10; targetColumn++) {
														var opts = $.tablesorter.filter
																.getOptionSource(
																		table,
																		targetColumn);
														var html = '<li class="espacamentoFiltros">'+ opts.map(function(elem) {
															if(targetColumn == 6) {
																if(elem.text == 'AltaBaixaMédia')
																	return '<input type="checkbox" data-column="' + targetColumn + '" value="' + elem.text + '">Alta';
																if(elem.text == 'BaixaMédiaAlta')
																	return '<input type="checkbox" data-column="' + targetColumn + '" value="' + elem.text + '">Baixa';
																if(elem.text == 'MédiaBaixaAlta')
																	return '<input type="checkbox" data-column="' + targetColumn + '" value="' + elem.text + '">Média';
															} else {
																if (targetColumn != 5 && targetColumn != 6 && targetColumn != 7) {									
																	return '<input type="checkbox" data-column="' + targetColumn + '" value="' + elem.text + '">' + elem.text;
																}
															}																																											
														}).join('</li><li class="espacamentoFiltros">') + '</li>';
														
														$('.search' + targetColumn) .append(html);
													
														$('.search' + targetColumn).on('ifChanged', ':checkbox', function(e) {														
															if ($(this).is(":checked")) {
																search[$(this).data("column")].push($(this).val());
															} else {
																search[$(this).data("column")].splice(search[$(this).data("column")].indexOf($(this).val()), 1);
															}
