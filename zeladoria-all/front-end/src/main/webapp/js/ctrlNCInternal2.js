var moduleNC = (function($, module) {
	...
	var $messageTpl = $("#success-template").html();
...
//			_render($comboEcTpl, $selectEntityCategory, resp.ec);
			...
			if (typeof (FileReader) != "undefined") {
				for (var i = 0; i < countFiles; i++) {
					...
					reader.onload = function(e) {
						...
						$(img).attr("src", e.target.result);
						$(img).addClass("thumb-image");
						upload1Base64 = resize(e.target.result);
					}
					image_holder.show();
					...
					if (typeof (FileReader) != "undefined") {
						for (var i = 0; i < countFiles; i++) {
							...
							reader.onload = function(e) {
								...
								$(img).attr("src", e.target.result);
								$(img).addClass("thumb-image");
								upload2Base64 = resize(e.target.result);
							}
							image_holder.show();
							reader.readAsDataURL($(this)[0].files[i]);
						}
					...
					if (typeof (FileReader) != "undefined") {
						for (var i = 0; i < countFiles; i++) {
							...
							reader.onload = function(e) {
								...
								$(img).attr("src", e.target.result);
								$(img).addClass("thumb-image");
								upload3Base64 = resize(e.target.result);
							}
							image_holder.show();
							reader.readAsDataURL($(this)[0].files[i]);
						}
	...
	function save() {
		if (!$formModule.valid()) {
			modal("Chamado", "Por favor, preencha os campos em vermelho",3);
			return;
		}
		if (getUrlParam('cpf') === "")
			$.ajax({
