...
function loadInputs(data) {
	...
	var call = JSON.parse(data.jsonList);
	var medias = call.medias;
	...
	if (call.noMidia === false) {
		if (medias !== undefined) {
			for (var ii = 0; ii < medias.length; ii++)
				$('#viewMedia').append(
						"<a type = 'button' target = '_self' class='view-media'><img src="
								+ base64(medias[ii])
		...
		$('#viewMedia')
				.append(
						"<a type = 'button' target = '_self' class='view-media'>"
								+ "<img src='assets/login/images/login-logo.png' height='130' width='130'></a>&nbsp&nbsp");
	}
	...
	$("#cNeighborhood").append(call.addressId.neighborhoodId.neighborhood_name);
...
$(document).ready(
		function() {
			...
			var hash = geturl('code');
			findByIdADD('../rest/webuser/child', hash);
