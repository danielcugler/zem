...
$(document).ready(function(){
	...
	scri.src = "config/menu.json";
	$("head").append(scri);
	
	var list = $('#list li').map(function() {
		return $(this).text();
	});
	//if($(".userPhoto").attr("src") ===""){
	function getUserPhoto(){
	$.ajax({
		type : 'GET',
		url : "../rest/systemuser2" +"/"+ $("#username").text(),
		dataType : "json",
		contentType : "application/json"
	}).done(function(resp, status, xhr) {
		updateUserPhoto(resp.photoPath);
		$("#userEntity").text(resp.worksAtEntity.entityId);
	}).fail(function(jqXHR, textStatus) {
	});
	}
	
	function getUserPhotoMinio(){
	$.ajax({
		type : 'GET',
		url : "../rest/systemuser2/minio/"+ $("#username").text(),
		dataType : "json",
		contentType : "application/json"
	}).done(function(resp, status, xhr) {
		updateUserPhoto(resp.url);
	}).fail(function(jqXHR, textStatus) {
	});
	}
	getUserPhotoMinio();
	//}
	//getUserPhoto();
	//var usuario = findById("../rest/systemuser" +"/"+ $("#username").text());
	
	//updateUserPhoto(usuario.photoPath);
	 
	var str='<div id="sidebar-menu"><ul id="leftMenuu"> <li><a href="index.jsp"	class="item1${pageContext.request.requestURI eq \'/index.jsp\' ? \' active\' : \'\'}"><i class="icon-home-3"></i><span>Home</span></a></li>'; 
	...
	function parseMenu(ul, menu) {
		...
		for (var i=0;i<menu.length;i++) {
			//sozinho
			if (menu[i].sub==null){
				if($.inArray(menu[i].role[0], list) >= 0){
					str+='<li><a href="'+menu[i].link+'" class="item'+count+'${pageContext.request.requestURI eq \''+menu[i].link+'\' ? "\' active\' : \'\'}"><i class="'+menu[i].icon+'"></i><span>'+menu[i].name+'</span></a></li>';
				}
				count++;
			}
			else{
				...
				for (var j=0; j<menu[i].sub.length; j++) {
					if($.inArray(menu[i].role[j], list) >= 0){
						permissao = true;
						break;
					}
				}
				...
				if(permissao){
					str+= '<li class="has_sub"><a href="'+menu[i].link+'"><i class="'+menu[i].icon+'"></i><span>'+menu[i].name+'</span><span class="pull-right"><i class="fa fa-angle-down"></i></span></a>';
					str+='<ul id="submenu">';
					count++;
					for (var ii=0;ii<menu[i].sub.length;ii++) {
						//filhos
						if($.inArray(menu[i].sub[ii].role[0], list) >= 0){
							str+='<li><a href="'+menu[i].sub[ii].link+'" class="item'+count+'${pageContext.request.requestURI eq \''+menu[i].sub[ii].link+'\' ? "\' active\' : \'\'}"><span>'+menu[i].sub[ii].name+'</span></a></li>';
						}
						count++;
					}
					str+="</ul></li>";
	...
	$(".editUser").on("click", function(){
		window.location = "addUser.jsp?botao=2&id=" + $("#username").text();
	});
