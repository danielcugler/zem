...
function loadInputs(data) {
	...
	if (data.observation != null) {
		$('#CComments').val(data.observation.information);
		$('#CCommentsId')
	...
	$('#COrigem').val(data.callSource);
	$('#CDate').val(data.creationDate);
	if(data.updatedOrModeratedBy != undefined)
		$('#CUser').val(data.updatedOrModeratedBy.systemUserUsername);
	else
		$('#CUser').val("Cidadão");
	...
	$(
			"#selectPriority option[value='"
					+ getPriority(data.priority) + "']").prop(
			'selected', true);
	$(
			"#selectCallClassification option[value='"
					+ data.callClassificationId.callClassificationId
					+ "']").prop('selected', true);
...
$(document)
		.ready(
				function() {
					...
					function getCall(id) {
						var callclaid;
						var callClassification = $("#selectCallClassification option:selected")
						.val();
						...
						if(callClassification == "0"){
							...
							if(callClassification == "1"){
								...
								if(callClassification == "2"){
									callclaid = 2;
								}else{
									if(callClassification == "3"){
										callclaid = 3;
									}else{
										if(callClassification == "4"){
											callclaid = 4;
										}else{
											if(callClassification == "5"){
												callclaid = 5;
											}else{
												if(callClassification == "6"){
													callclaid = 6;
												}
											}
										}
