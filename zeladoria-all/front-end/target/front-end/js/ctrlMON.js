var moduleMON = (function($, module) {
	...
	function tableTpl() {
		var string = "{{#each .}}"
				...
				+ "<td align='center'><div data-toggle='collapse' data-target='#{{unsolvedCallId}}' data-parent='#accordion' class='accordion-toggle cursor'><i class='glyphicon glyphicon-chevron-down'></i></div></td>"
		...
		if (roleEnabled)
			string += "{{#ifCond callStatus '===' 'Ativo'}}"
					+ "<a data-idp=\"{{parentCallId.unsolvedCallId}}\" data-id=\"{{unsolvedCallId}}\" data-enabled=\"{{enabled}}\" type = 'button' data-toggle='tooltip' title='Rejeitar' class='btn btn-xs btn-danger ed-button'><i class='glyphicon glyphicon-remove'></i></a>"
					+ "{{else}}"
					+ "<a data-idp=\"{{parentCallId.unsolvedCallId}}\" data-id=\"{{unsolvedCallId}}\" data-enabled=\"{{enabled}}\" type = 'button' data-toggle='tooltip' title='Ativar' class='btn btn-xs btn-success ed-button'><i class='glyphicon glyphicon-ok'></i></a>"
					+ "{{/ifCond}}";
		if (roleSend)
			string += "{{#ifCond callStatus '===' 'Ativo'}}"
		...
		if (roleEdit)
			string += "<a href=\"../editCall.jsp?id={{unsolvedCallId}}\" type = 'button' target = '_self' data-toggle='tooltip' title='Editar' class='btn btn-xs btn-primary edit-button'><i class='glyphicon glyphicon-pencil'></i></a>";
		string += "{{#ifCond callStatus '===' 'Ativo'}}"
				+ "<a data-idp=\"{{parentCallId.unsolvedCallId}}\" data-id=\"{{unsolvedCallId}}\" data-enabled=\"{{enabled}}\" type = 'button' data-toggle='tooltip' title='Rejeitar' class='btn btn-xs btn-danger enabled-button ed-button'><i class='glyphicon glyphicon-remove'></i></a>";
		string += "{{else}}"
				+ "<a data-idp=\"{{parentCallId.unsolvedCallId}}\" data-id=\"{{unsolvedCallId}}\" data-enabled=\"{{enabled}}\" type = 'button' data-toggle='tooltip' title='Ativar' class='btn btn-xs btn-success disabled-button ed-button'><i class='glyphicon glyphicon-ok'></i></a>"
...
/*
	function toggleChevron(e) {
	 ...
	 $(document).on('shown.bs.collapse','.accordion', function(){alert('1');});
	$('.cursor').on('shown.bs.collapse', toggleChevron);
	*/
		...
		$.ajax(
				{
					...
					contentType : "application/json",
					dataType : "json",
					// 'data' : {'bmid':id,'username':$('#username').text()},
					beforeSend : function(xhr) {
						xhr.setRequestHeader(header, token);
					}
