var ctrlSU = (function($, module) {
	...
	var $selectSup = $searchModule.find("#selectPerfil");
	...
	function tableTpl(roleEdit, roleEnabled) {
		...
		if (roleEdit)
			string += "<a href=\"../addUser.jsp?id={{systemUserUsername}}\" type = 'button' target = '_self' data-toggle='tooltip' title='Editar' class='btn btn-xs btn-primary edit-button'><i class='glyphicon glyphicon-pencil'></i></a>";
		if (roleEnabled)
			string += "{{#ifCond enabled '===' 0}}"
					+ "<a type = 'button' data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{systemUserUsername}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" data-toggle='tooltip' title='Inativar' class='btn btn-xs btn-danger ed-button enabled-button'><i class='glyphicon glyphicon-remove'></i></a>"
					+ "<a type = 'button' data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{systemUserUsername}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" style=\"display: none;\" data-toggle='tooltip' title='Ativar' class='btn btn-xs btn-success ed-button disabled-button'><i class='glyphicon glyphicon-ok'></i></a>"
					+ "{{else}}"
					+ "<a type = 'button' data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{systemUserUsername}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" style=\"display: none;\" data-toggle='tooltip' title='Inativar' class='btn btn-xs btn-danger ed-button enabled-button'><i class='glyphicon glyphicon-remove'></i></a>"
					+ "<a type = 'button'  data-row-index='{{@index}}' data-link=\"{{getLink links 'ed'}}\" data-id=\"{{systemUserUsername}}\" data-name=\"{{name}}\" data-enabled=\"{{enabled}}\" data-toggle='tooltip' title='Ativar' class='btn btn-xs btn-success ed-button disabled-button'><i class='glyphicon glyphicon-ok'></i></a>"
					+ "{{/ifCond}}";
	...
	function makeCombos() {
		$.ajax({
			type : 'GET',
			url : "../rest/systemuserprofile2/combos",
			dataType : "json",
			contentType : "application/json"
		}).done(function(resp, status, xhr) {
			_render($comboSupTpl, $selectSup, resp);
	...
	function modal(title, body) {
		...
		//$modalE.modal('show');
		$modalE.modal({show:true, backdrop:true});
	}
	...
	function setFilters(name, perfil, enabled) {
		$('#inName').val(name);
	...
	function search(event) {
		...
		//page = $("#pagination").pagination('getCurrentPage') || event.data.param;
		searchMM(page);
	}
	...
	function changeEnabled(event) {
		...
		// var body = (enabled == "Enabled") ? "Perfil " + name
		// + " inativado com sucesso!" : "Perfil " + name
		// + " ativado com sucesso!";
		$.ajax({
			type : 'PUT',
